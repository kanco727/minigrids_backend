...
async def create_site(payload: schemas.SiteCreate, db: AsyncSession = Depends(get_db)):
    ...
    cols = ["projet_id","localite","score_acces","niveau_securite","population_estimee","notes","statut","visibilite"]
    vals = [":projet_id",":localite",":score_acces",":niveau_securite",":population_estimee",":notes",":statut",":visibilite"]

    if point_wkt is not None:
        cols.append("point"); vals.append("ST_GeomFromText(:point_wkt,4326)")
        d["point_wkt"] = point_wkt
    if zone_wkt is not None:
        cols.append("zone"); vals.append("ST_GeomFromText(:zone_wkt,4326)")
        d["zone_wkt"] = zone_wkt

    q = text(f"""
      INSERT INTO site ({", ".join(cols)}) VALUES ({", ".join(vals)})
    ...
    await db.commit()
    if not row: raise HTTPException(400, "Insertion site échouée")
    return dict(row)
...
async def update_site(site_id: int, payload: schemas.SiteUpdate, db: AsyncSession = Depends(get_db)):
    ...
    # Champs simples
    for k in ["projet_id","localite","score_acces","niveau_securite","population_estimee",
              "notes","statut","visibilite"]:
        if k in d:
            sets.append(f"{k}=:{k}")
    ...
    d["id"] = site_id
    q = text(f"""
      UPDATE site SET {", ".join(sets)}, maj_le=now()
      WHERE id=:id
      RETURNING id, projet_id, localite,
                ST_AsText(point) AS point_wkt,
                ST_AsText(zone)  AS zone_wkt,
...
async def delete_site(site_id: int, db: AsyncSession = Depends(get_db)):
    val = (await db.execute(text("DELETE FROM site WHERE id=:id RETURNING id"), {"id": site_id})).scalar()
    await db.commit()
    if val is None: raise HTTPException(404, "Site introuvable")
